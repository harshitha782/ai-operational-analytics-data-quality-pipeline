...
st.set_page_config(page_title="NovaRetail Operations Analytics", layout="wide")

DB_PATH = Path("data/processed/novaretail.db")

if not DB_PATH.exists():
    st.error("Database not found. Run: python run_project.py")
    st.stop()
...
SELECT ROUND(100.0 * AVG(is_late), 2) AS late_rate
...
SELECT ROUND(
    ...
    NULLIF((SELECT COUNT(*) FROM order_items), 0), 2
) AS return_rate
FROM returns r
""", con).iloc[0, 0]
...
with left:
    ...
    st.plotly_chart(
        px.line(monthly, x="month", y="revenue", markers=True),
        use_container_width=True
    )
with right:
    st.subheader("Revenue by Region")
    st.plotly_chart(
        px.bar(region, x="region", y="revenue"),
        use_container_width=True
    )
...
with left:
    ...
    st.plotly_chart(
        px.bar(carrier, x="carrier", y="late_rate"),
        use_container_width=True
    )
with right:
    st.subheader("Return Rate by Category")
    st.plotly_chart(
        px.bar(category_returns, x="category", y="return_rate"),
        use_container_width=True
    )
...
if anomaly_path.exists():
    ...
    st.info("Run python run_project.py to generate anomaly results.")
